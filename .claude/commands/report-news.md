...
- Sort articles newest first within each keyword group
...
Then group articles **by keyword** in the order defined by `news.keywords` in `dna-config.yaml`. For each keyword with results, add a small keyword callout banner in its assigned color, followed by the matching articles newest first:
...
Pair articles into rows of 2 using `<columns>`. If there's an odd number for a keyword, the last article gets a full-width block (no `<columns>` wrapper). Convert `published_at` to relative time format ("2 hours ago", "5 hours ago"). Omit keywords that returned no results.
...
Categories are displayed in the order defined by `categories` in `dna-config.yaml`. Each category is a **colored callout banner** followed by articles listed below it (not inside the callout). Use these category colors:
